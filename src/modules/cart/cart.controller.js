...
const addToCart = catchError(async (req, res, next) => {
  ...
  let existingItem = isCartExist.cartItems.find(
  ...
  res
    ...
    .json({ message: "Cart updated successfully ✅", cart: isCartExist });
});
...
const getLoggedInUserCart = catchError(async (req, res, next) => {
  ...
  !cart &&
    res.status(statusCode.NOT_FOUND).json({ message: "Cart Not Found!" });
  cart &&
    res
      .status(statusCode.OK)
